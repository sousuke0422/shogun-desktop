...
pub fn render_terminal_tab(
    ...
    v_flex()
        ...
        // shortcuts. action_context routes them to the terminal focus handle,
        // i.e. the pane's TERMINAL_KEY_CONTEXT on_action handlers. Attached to
        // the OUTER non-scrolling flex, never to the scroll container: an open
        // menu injects a window-sized absolute subtree as a child, taffy counts
        // absolute children toward a scroll container's content size (inflow ⊔
        // absolute — the same trap as the 8px micro-scroll documented on the
