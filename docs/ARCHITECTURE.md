...
パスワード取得の優先順位:
  1. keyring::Entry::get_password()  ← OS キーストア (暗号化)
       失敗時: 最大3回リトライ (500ms 間隔) — デーモン起動遅延・ロック解除待ちを吸収
       3回失敗後: 平文フォールバックへ
  2. settings.toml [ssh] password    ← 平文フォールバック
  3. (なし) → 接続時にプロンプト
```

keyring 3回リトライ後も失敗した場合、以下の3ボタンダイアログを表示する。
（GitHub Copilot のトークン管理 UI からの着想）
...
- **平文で保存**: settings.toml に書き込む（利便性優先）
- **メモリ上のみ**: 起動中のみ保持、終了時に破棄（セキュリティ優先）
- **再試行**: keyring アクセスを再度試みる（ロック解除待ちなど）
...
起動直後に以下の2つをチェックし、非対応環境では即座にエラーダイアログを出して終了する。

**① Wine 検出**

`ntdll.dll` の `wine_get_version` エクスポートを確認する。

```rust
// ntdll に wine_get_version が存在すれば Wine 環境
let is_wine = get_proc_address("ntdll.dll", "wine_get_version").is_some();
```

検出時のメッセージ:
```
Wine 環境が検出されました。
Linux / macOS ネイティブ版をご利用ください。
```

なお Wine が `wine_get_version` を秘匿している場合は検出できない。
その場合は ConPTY / GPUI 描画の時点でクラッシュするが、
こちらは誠実に検出を試みており責任は Wine 側にある。

**② Windows バージョン**

ConPTY は Windows 10 build 17763 (バージョン 1809) 以降が必須。
`RtlGetVersion` で取得し、それ以前なら弾く。
...
検出時のメッセージ:
```
Windows 10 バージョン 1809 (build 17763) 以降が必要です。
現在のバージョン: {major}.{minor}.{build}
```

### エージェントタブ刷新 (TODO)

現在のエージェントタブは `agent_status.sh` の生テキストをそのまま表示している。
以下の方針で構造化UIに刷新する。

**基本方針: YAML駆動**

`capture-pane` は tmux クライアントサイズに依存するため、
別ターミナルが接続中だとレイアウトが崩壊する。
SSH経由でYAMLを直接取得する方式を採用する。

```
取得ソース (SSHで cat):
  queue/tasks/{agent}.yaml       → task_id, status
  queue/inbox/{agent}.yaml       → 未読メッセージ数
  queue/reports/{agent}_report.yaml → 最終報告時刻・summary
```
...
```
┌─────────────┐ ┌─────────────┐ ┌─────────────┐
│ karo        │ │ gunshi      │ │ ashigaru1   │
│ cmd_176     │ │ idle        │ │ cmd_176 QC  │
│ assigned    │ │ inbox: 0    │ │ done        │
│ inbox: 1    │ │ 11:32更新   │ │ inbox: 0    │
└─────────────┘ └─────────────┘ └─────────────┘
```

**将来: ビュー切り替えトグル**

YAML構造化ビューをデフォルトとし、トグルで `capture-pane` 生テキストビューに切り替え可能にする。
実機使用後にどちらが実用的か判断してから実装する。
