...
impl DirectWriteState {
    ...
    fn generate_font_fallbacks(
        ...
        unsafe {
            ...
            // PATCHED (shogun-desktop): resolve fallback families against the
            // CUSTOM font collection first — fonts embedded via `add_fonts`
            // live there, not in the system collection, and were silently
            // skipped. When a family comes from the custom collection, that
            // collection must also be passed to AddMapping: resolution at
            // layout time otherwise uses the layout's base collection, where
            // the family may not exist.
            let custom_font_set = self.custom_font_collection.GetFontSet()?;
            let system_font_set = self.system_font_collection.GetFontSet()?;
            let custom_collection_base: IDWriteFontCollection =
                self.custom_font_collection.cast()?;
            for family_name in fallbacks.fallback_list() {
                let mut matched = None;
                for (font_set, collection) in [
                    (&custom_font_set, Some(&custom_collection_base)),
                    (&system_font_set, None),
                ] {
                ...
                let Some((fonts, mapping_collection)) = matched else {
