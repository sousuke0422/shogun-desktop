...
| `b47bc70` | FontRun merging (`text_system.rs`) | `layout_line` merges FontRuns on font alone; upstream also split on any decoration (color/underline/strikethrough) change | Terminal SGR-churn floods (per-char colors) made the layout-cache key color-dependent and re-shaped per fragment — paint p95 ~20ms at 4K. Consequence, accepted: ligatures may form across decoration boundaries, painted with the first byte's color |
| `3056dd5` | Drag preview skipped in popups (`window.rs`) | `Window` remembers its `WindowKind`; `draw` no longer paints `App::active_drag` in a `PopUp` | `active_drag` is App-global, so every window drawn during a drag paints its own copy of the preview. The follower popup exists to BE that preview, so it drew a second one over itself — two overlapping chips |
