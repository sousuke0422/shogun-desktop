...
pub trait Handler {
    ...
    /// DECSLRM - Set the left and right margins.
    ///
    /// PATCHED (rikka): upstream routed every `CSI s` to
    /// [`Handler::save_cursor_position`] and threw the parameters away, so an
    /// application could not set left/right margins at all — tmux uses them to
    /// scroll a single pane's rectangle, and without them the scroll takes the
    /// full width and wipes the neighbouring panes.
    ///
    /// `CSI s` is ambiguous: DECSLRM while the application enabled DECLRMM
    /// (private mode 69), SCOSC otherwise. Only the terminal tracks that mode,
    /// so the parameters arrive here and the implementor decides — an
    /// implementation that does not support margins should call its own
    /// `save_cursor_position`, which is what the default does not do here
    /// because this trait cannot see `self`'s mode.
    fn set_left_right_margins(&mut self, _left: Option<usize>, _right: Option<usize>) {}
    ...
    T: Timeout,
{
    ...
    fn csi_dispatch(
        ...
        match (action, intermediates) {
            ...
            // PATCHED (rikka): DECSLRM. The parameters used to be dropped and
            // this was unconditionally SCOSC; the terminal now decides between
            // the two, because only it knows whether DECLRMM (?69) is set.
            ('s', []) => {
                let left = params_iter.next().map(|param| param[0] as usize).filter(|&p| p != 0);
                let right = params_iter.next().map(|param| param[0] as usize).filter(|&p| p != 0);
                handler.set_left_right_margins(left, right);
            },
