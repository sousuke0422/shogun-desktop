...
> 実測で確認。**fork の建前動機は消えた**。残る動機は下の果実欄のみ。
> **殿裁定 2026-09-07: 棚上げ**（副次動機だけでは見合わない。再開条件は
> 果実欄のどれかが実害として顕在化した時）。

- [ ] **OpenConsole fork（殿決定 2026-08-10 → 棚上げ 2026-09-07）** —
      当初の芯「DA1 ローカル応答を転送に組み替える」は**不要と判明**
      （ホストは既に転送・中継している。上記）。再開するなら動機は次の
      果実欄に限られる。
