...
impl TextView {
    ...
    /// PATCHED: read the live selection of the TextView rendered under `id`.
    ///
    /// The state entity is keyed by element id (see `markdown`/`html` below),
    /// so the same key retrieves the same entity. Everything about the
    /// selection is `pub(crate)`, which is right for the widget's own API but
    /// leaves an embedder's context menu unable to offer "copy selection" —
    /// this is the one deliberate crack in that wall. Returns `None` when
    /// nothing is selected.
    ///
    /// MUST be called during a view's render: `use_keyed_state` resolves the
    /// current view, which only exists there. From any deferred context
    /// (a context-menu builder, an event handler) it panics — read it in
    /// render and freeze the value into the closure instead.
    pub fn selection_of(
        ...
        if self.selectable {
            ...
            window.on_mouse_event({
                ...
                move |event: &MouseDownEvent, phase, _, cx| {
                    ...
                    // PATCHED: left button only. This fired for ANY button,
                    // so a right-click restarted the selection at the click
                    // point — collapsing exactly the selection a context
                    // menu's "copy selection" is about to act on.
            ...
            if has_selection {
                ...
                window.on_mouse_event({
                    ...
                    move |event: &MouseDownEvent, _, _, cx| {
                        ...
                        // PATCHED: same rule for the outside-clear — a right
                        // mousedown must never mutate the selection.
                        if event.button != gpui::MouseButton::Left {
                            return;
                        }
