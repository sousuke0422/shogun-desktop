# E2E: cursor shapes (DECSCUSR) against a live build.
# Opens the shell window, emits OSC 9;4 progress states with printf, and
# screenshots the taskbar button strip after each: 50% (green fill),
# indeterminate (marquee), then cleared. Keep this file ASCII-only.
...
# Crop of the shell window's upper-left area (prompt + cursor live there).
function Shot-Prompt([string]$name) {
    $rect = New-Object E2E+RECT
    [E2E]::GetWindowRect([IntPtr]$script:shellHwnd, [ref]$rect) | Out-Null
    $w = 1400; $h = 320
...
Shot-Prompt 'cur0-block.png'
Run-Cmd 'printf "\033[5 q"'
Start-Sleep -Seconds 1
Shot-Prompt 'cur1-beam.png'
Run-Cmd 'printf "\033[3 q"'
Start-Sleep -Seconds 1
Shot-Prompt 'cur2-underline.png'
Run-Cmd 'printf "\033[?25l"'
Start-Sleep -Seconds 1
Shot-Prompt 'cur3-hidden.png'
Run-Cmd 'printf "\033[?25h\033[2 q"'
